...
import { BandAgentCallbackHandler } from './handlers/callbacks';
...
function configureToolNameRegistry(
	...
	for (const callback of callbacks) {
		if (callback instanceof BandAgentCallbackHandler) {
 ...
 *
 * @param memory - BandMemory instance to inject
 * @param callbacks - Array of callback handlers
 */
function configureMemoryOnCallbacks(memory: BandMemory, callbacks: CallbackHandlers): void {
	for (const callback of callbacks) {
		if (callback instanceof BandAgentCallbackHandler) {
