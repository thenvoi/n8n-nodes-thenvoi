...
import { ChatMessageMention, BandCredentials } from '@lib/types';
...
/**
 * LangChain callback handler that sends events to Band chat in real-time
 * Thin orchestrator that delegates to pure handler functions
 */
export class BandAgentCallbackHandler extends BaseCallbackHandler {
	/** Handler identifier for LangChain */
	name = 'BandAgentCallbackHandler';
	...
	constructor(
		...
		executionContext.logger.info('BandAgentCallbackHandler initialized', {
			chatId,
			taskId: this.taskId,
			streaming: {
	 ...
	 */
	setMemory(memory: BandMemory): void {
		this.memory = memory;
	}
