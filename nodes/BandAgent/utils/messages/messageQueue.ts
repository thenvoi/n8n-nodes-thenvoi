import { sendEventToBand, sendTextMessageToBand } from '@lib/api/messages';
 ...
 *
 * Ensures messages are sent to the Band API in order by maintaining
...
async function sendByType(
	...
	if (isEventType(messageType)) {
		await sendEventToBand(httpClient, chatId, messageType, content, metadata);
	} else {
		await sendTextMessageToBand(httpClient, chatId, content, mentions || []);
	}
}
