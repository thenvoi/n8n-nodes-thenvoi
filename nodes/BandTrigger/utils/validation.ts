...
export function validateCredentials(
	...
	if (!credentials) {
		triggerContext.logger.error('Band Trigger: No credentials provided');
		throw new NodeOperationError(triggerContext.getNode(), 'Band credentials are required');
	}
}
...
export function validateConfig(config: TriggerConfig, triggerContext: ITriggerFunctions): void {
	try {
		...
		logError(triggerContext.logger, 'Band Trigger: Configuration validation failed', error);
